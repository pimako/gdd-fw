...
4. `.specs/PERSONAS.md`(プロジェクト単位のペルソナロスター)が存在しない場合、`spec-review`/`spec-verify`/`spec-discuss` が暫定ロスールで動くことになる旨を伝え、`spec-team` を先に実行することを推奨する(ただし必須ではない。小規模な変更ではスキップしてよい)。
5. ユーザーに、次は `spec-spec` スキル(要件定義)に進むことを伝える。まとめて全フェーズを自動で進めたい場合は `spec-run` を使うよう案内する。
