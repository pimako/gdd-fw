...
**ISTQB的位置づけ**: `gate-test` がコードを実行して確認する**動的テスト**(Integration/System/Acceptance Testing)であるのに対し、この`gate-check`の複数ペルソナレビューはコードを実行せず成果物を読んで評価する**静的テスト**(レビュー)にあたる。具体的には、独立した複数のレビュアー(ペルソナ)が並行して指摘を出し、司会役(このスキル自身)が指摘を統合するという構造上、ISTQBでいう**テクニカルレビュー/インスペクション**に相当する(参加者が事前に準備し、役割を分担し、指摘を記録・追跡する形式的なレビュー)。動的テスト(gate-test)と静的テスト(gate-check)の両方を通過して初めてGateがPASSする、という設計。
